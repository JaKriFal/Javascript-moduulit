...
for(let j = 0; j < picArray.length; j++) { //loop through all array elements

  //figure and it's contents:
  let f = document.createElement('figure')
  let i = document.createElement('img')
  i.src = picArray[j].image.medium
  i.alt = picArray[j].title
  let fcap = document.createElement('figcaption')
  let ftext = document.createTextNode(picArray[j].caption)
  fcap.appendChild(ftext)
  f.appendChild(i)
  f.appendChild(fcap)

  //header:
  let h = document.createElement('h2')
  let htext = document.createTextNode(picArray[j].title)
  h.appendChild(htext)

  //p-element:
  let p = document.createElement('h2')
  const ptext = document.createTextNode(picArray[j].description)
  p.appendChild(ptext)

  //create article, add class and append children:
  let a = document.createElement('article')
  a.classList.toggle('card')
  a.appendChild(h)
  a.appendChild(f)
  a.appendChild(p)
